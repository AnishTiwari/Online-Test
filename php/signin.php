...
//preventing to store duplicate id and email in database
    $dupquery= mysqli_query($con,"SELECT * FROM first_year WHERE student_ID = '$userid' OR email = '$email' ") or die(mysqli_error($con));
...
{//CHECKING FOR DUPLICANCIES IN DATABASE...
    ...
    window.location.href='../html/welcome.html#parentHorizontalTab_agile2';
    </SCRIPT>");
    ...
    {
    echo($password_hash);
         $query= mysqli_query($con,"INSERT INTO first_year(student_name, student_ID, password, email, phoneno) VALUES ('$username', '$userid', '$password_hash', '$email', '$phoneno') ") or die(mysqli_error($con));
